...
**实现要点**  
- 每步调用 `observation.step_begin()/step_end()` 与 `ProfilerObservation.snapshot()` 获取观测快照并写入 `obs_metrics.csv`。【F:train.py†L196-L277】  
- 观测快照喂给 `PhaseInference.update()` 产生适用性状态。【F:train.py†L254-L268】  
- 将 `phase_state` 与 `observation_stats` 传入 `maybe_checkpoint()`，由策略控制器驱动实际触发与机制选择。【F:train.py†L280-L297】  
